...
class Master_db(models.Model):   
    ...
    # week_start = models.DateField(default=get_week_start)
    # week_end = models.DateField()
    # week_start = models.DateTimeField(previous_week_range(date))

    @property
    def week_start(self):
        week_start =  self.date + datetime.timedelta(self.date.weekday(), weeks =-1)
        return week_start

    # @property
    # def week_end(self):
    #     # week_end = self.date +datetime.timedelta(-self.date.weekday() - 1)
    #     # return week_end
    #     now = arrow.now()
